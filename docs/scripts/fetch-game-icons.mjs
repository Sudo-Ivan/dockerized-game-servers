...
export async function fetchGameIcons() {
	...
	/** @type {Record<string, string>} */
	const icons = {}
	/** @type {Record<string, 'contain' | 'cover'>} */
	const iconFit = {}
	...
	for (const slug of pages) {
		try {
			...
			const fitFromFrontmatter =
				attrs.iconFit === 'contain' || attrs.iconFit === 'cover' ? attrs.iconFit : null
			...
			let fit = fitFromFrontmatter
			if (!fit) {
				if (
					ICON_FIT_CONTAIN.has(slug) ||
					source.startsWith('direct:') ||
					source.startsWith('wikipedia:')
				) {
					fit = 'contain'
				} else {
					fit = 'cover'
			...
			report.push({
				slug,
				status: 'ok',
				detail: `${source}${asset.appId ? ` app=${asset.appId}` : ''}`,
			})
		} catch (error) {
...
export const gameIconFit = ${JSON.stringify(iconFit, null, 2)}
