...
- Server page title icons from Steam CDN (or Wikipedia/direct fallback), via scripts/fetch-game-icons.mjs. Non-Steam art uses `object-fit: contain` (see `gameIconFit` in generated output).
...
Optional page frontmatter for icon resolution when automatic discovery fails:
