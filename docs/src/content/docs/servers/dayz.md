...
This image installs the DayZ dedicated server through SteamCMD. You must use a Steam account that owns DayZ for both the server and workshop mod downloads. Anonymous Steam login cannot install the server.
...
- Keep a data folder for the server install, config, profiles, and mods
- Open UDP ports 2302 through 2306
- Set STEAM_USERNAME, STEAM_PASSWORD, and optional STEAM_GUARD_CODE. Use a dedicated Steam account, not your main gaming account
...
| --- | --- | --- |
| 2302 to 2306 | UDP | Game and Steam query (DAYZ_PORT is the main game port) |
...
Set DAYZ_FORCE_UPDATE to true to reinstall on the next start. You can also run ./tools/gs update dayz. See [Ops](/guides/ops/) for backup and restore too.
...
Run on the host (or docker compose exec with Steam credentials) with install dir set to your server data folder:
...
Load order matters. Community Framework and other dependencies must appear before mods that require them. Restart the server after any mod change.

### 5. Update mods

Stop the server, re-run workshop_download_item for each ID, refresh symlinks if needed, copy any new .bikey files, then start again. The [Bohemia wiki](https://community.bohemia.net/wiki/DayZ:Hosting_a_Linux_Server) shows an update.sh pattern that removes old symlinks and keys before re-linking.
